...
| Method |  | $$v$$ | $$\rho$$ | $$J_s$$ |
| *A* | <img src="../img/jpsreport_Method_A.png" width=100 align=center> |  $$\langle v \rangle_{\Delta t}=\frac{1}{N^{\Delta t}}\sum_{i=1}^{N^{\Delta t}} v_i(t)$$ | | $$\langle J \rangle_{\Delta t}=\frac{N^{\Delta t}}{t_N^{\Delta t} - t_1^{\Delta t}}$$ |
| *B* | ![Method_B]({{ site.baseurl }}/img/jpsreport_Method_B.png) | $$\langle v \rangle_i=\frac{\Delta x}{t_\text{out}-t_\text{in}}$$ | $$\langle \rho \rangle_i=\frac{1}{t_\text{out}-t_\text{in}}\cdot\int_{t_\text{in}}^{t_\text{out}} \frac{N^\prime(t)}{b_\text{cor}\cdot\Delta x}dt$$ | |
| *C* | ![Method_C]({{ site.baseurl }}/img/jpsreport_Method_C.png) | $$\langle v \rangle_{\Delta x}=\frac{1}{N}\sum_{i=1}^{N}{v_i(t)}$$ | $$\langle \rho \rangle_{\Delta x}=\frac{N}{b_\text{cor}\cdot\Delta x}$$ | |
| *D* | ![Method_D]({{ site.baseurl }}/img/jpsreport_Method_D.png) | $$\langle v \rangle_v=\frac{\iint{v_{xy}dxdy}}{b_\text{cor}\cdot\Delta x}$$ | $$\langle \rho \rangle_v=\frac{\iint{\rho_{xy}dxdy}}{b_\text{cor}\cdot\Delta x}$$ |  |
...
- Output data (in the output folder: `/Fundamental_Diagram/IndividualFD/`) in the output file `IFD_I_` with
