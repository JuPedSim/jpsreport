...
Several measurement areas can be given and distiguished with different `id`.

The parameter `zPos` is used to indicate the position of measuement area in z axis. `zPos` is useful for geometry with several stories.
...
## Methods
  ...
  Five different methods `method_A` to `method_D` and `Method_I` are integrated in the current
  version of `JPSreport` and can be chosen for the analysis. The four methods `method_A` to `method_D` are used to analyze the steady state.

| Method |  | $$v$$ | $$\rho$$ | $$J_s$$ |
| *A* | ![Method_A]({{ site.baseurl }}/img/jpsreport_Method_A.png | width=100) |  $$\langle v \rangle_{\Delta t}=\frac{1}{N^{\Delta t}}\sum_{i=1}^{N^{\Delta t}} v_i(t)$$ | | $$\langle J \rangle_{\Delta t}=\frac{N^{\Delta t}}{t_N^{\Delta t} - t_1^{\Delta t}}$$ |
| *B* | ![Method_B]({{ site.baseurl }}/img/jpsreport_Method_B.png | width=100) | $$\langle v \rangle_i=\frac{\Delta x}{t_\text{out}-t_\text{in}}$$ | $$\langle \rho \rangle_i=\frac{1}{t_\text{out}-t_\text{in}}\cdot\int_{t_\text{in}}^{t_\text{out}} \frac{N^\prime(t)}{b_\text{cor}\cdot\Delta x}dt$$ | |
| *C* | ![Method_C]({{ site.baseurl }}/img/jpsreport_Method_C.png | width=100) | $$\langle v \rangle_{\Delta x}=\frac{1}{N}\sum_{i=1}^{N}{v_i(t)}$$ | $$\langle \rho \rangle_{\Delta x}=\frac{N}{b_\text{cor}\cdot\Delta x}$$ | |
| *D* | ![Method_D]({{ site.baseurl }}/img/jpsreport_Method_D.png | width=100) | $$\langle v \rangle_v=\frac{\iint{v_{xy}dxdy}}{b_\text{cor}\cdot\Delta x}$$ | $$\langle \rho \rangle_v=\frac{\iint{\rho_{xy}dxdy}}{b_\text{cor}\cdot\Delta x}$$ |  |
...
`Method_I` can use for time-series analysis of individual data.
...
- Output data (in the output folder: `/Fundamental_Diagram/FlowVelocity/`) in the output files  `Flow_NT_traj_` and `FDFlowVelocity_traj_`.
...
Possible parameters are:
  ...
  - Output data (in the output folder: `/Fundamental_Diagram/TinTout/`)  in the output file `FDTinTout_traj_` with mean density and velocity of each pedestrian (PersID, $$\rho_i$$ and $$v_i$$).
...
- Output data (in the output folder: `/Fundamental_Diagram/Classical_Voronoi/`) in the output file `rho_v_Classic_traj_` with mean density and velocity of over time (frame, $$rho(t)$$, $$v(t)$$).
...
Method I use the definition of Method D. For definition see [Method D](2016-11-07-method-d.html). Method I can use for time-series analysis of individual data.
...
- Output data (in the output folder: `/Fundamental_Diagram/IndividualFD/`) in the output file `traj_` with
